  ...
  return (
    <div>
      ...
      <main className="max-w-md mx-auto px-6 py-16 text-center" data-testid="invite-page">
        <TicketCheck size={48} className="text-[#d4a373] mx-auto mb-4" />
        <h1 className="font-head text-3xl uppercase mb-3">Your seat is reserved</h1>
        {status === "joining" && (
          <div className="font-mono text-xs tracking-[0.3em] uppercase text-[#7a6a55]">Unrolling the carpet…</div>
        )}
        {status === "error" && (
          <>
            <div className="font-mono text-xs tracking-widest text-[#a04a2f] mb-4" data-testid="invite-error">{error}</div>
            <button
              onClick={() => navigate("/dashboard")}
              className="bg-[#d4a373] text-[#2b2118] font-mono tracking-[0.25em] uppercase text-sm px-6 py-3 hover:bg-[#c08456]"
            >
              Back to dashboard
            </button>
          </>
