...
function Row({ u, actions }) {
  return (
    <div className="flex items-center justify-between p-3 border border-[#7209b7]/30 bg-[#f0e6ff] rounded-sm">
      <div className="flex items-center gap-3 min-w-0">
        <div className="w-9 h-9 bg-[#7209b7]/20 border border-[#7209b7]/30 flex items-center justify-center font-head">
          {u.profile_image ? (
        ...
        <div className="min-w-0">
          ...
          <div className="font-mono text-[10px] text-[#6b5b84] truncate">{u.unique_id}</div>
        </div>
      </div>
  ...
  const tabBtn = (id, label, count) => (
    <button
      ...
      className={`px-4 py-2 font-mono tracking-[0.2em] uppercase text-xs border ${
        tab === id ? "border-[#7209b7] text-[#7209b7] bg-[#7209b7]/10" : "border-[#7209b7]/30 text-[#6b5b84] hover:text-[#1a0b2e]"
      }`}
    >
      {label} <span className="ml-2 text-[#7209b7]">{count}</span>
    </button>
  );
      ...
      <div className="max-w-[1000px]">
        <form onSubmit={doSearch} className="flex gap-2 mb-8">
          <div className="flex-1 relative">
            <Search size={16} className="absolute left-3 top-1/2 -translate-y-1/2 text-[#6b5b84]" />
            <input
              value={query}
              onChange={(e) => setQuery(e.target.value)}
              placeholder="Paste a unique ID e.g. CinemaSync_Name_DDMMYYYY"
              data-testid="friends-search-input"
              className="w-full bg-white border border-[#7209b7]/30 focus:border-[#7209b7] pl-10 pr-4 py-3 font-mono text-xs tracking-widest"
            />
          </div>
          <button
            disabled={searchLoading}
            data-testid="friends-search-button"
            className="bg-[#7209b7] text-[#1a0b2e] font-mono tracking-[0.2em] uppercase text-xs px-6 py-3 hover:bg-[#4a0580] disabled:opacity-60"
          >
            ...
            <Row
              ...
              actions={
                <button
                  ...
                  className="flex items-center gap-2 border border-[#7209b7] text-[#7209b7] px-3 py-2 text-xs font-mono tracking-widest uppercase hover:bg-[#7209b7]/10"
                >
        ...
        <div className="space-y-3">
          {tab === "friends" &&
            (friends.length === 0 ? (
              <div className="font-mono text-xs tracking-widest text-[#6b5b84] uppercase">Empty row. Add someone.</div>
            ) : friends.map((f) => (
              <Row
                key={f.id}
                u={f}
                actions={
                  <button
                    ...
                    className="flex items-center gap-2 border border-[#7209b7]/45 text-[#1a0b2e] px-3 py-2 text-xs font-mono tracking-widest uppercase hover:border-[#f72585] hover:text-[#f72585]"
                  >
          ...
          {tab === "in" &&
            (requestsIn.length === 0 ? (
              <div className="font-mono text-xs tracking-widest text-[#6b5b84] uppercase">No incoming requests.</div>
            ) : requestsIn.map((f) => (
              <Row
                key={f.id}
                u={f}
          ...
          {tab === "out" &&
            (requestsOut.length === 0 ? (
              <div className="font-mono text-xs tracking-widest text-[#6b5b84] uppercase">Nothing in the mail.</div>
            ) : requestsOut.map((f) => (
              <Row
                key={f.id}
                u={f}
